...
namespace MonsterSave.Runtime
{
    public class DictionaryAdapter : IDictionaryAdapter
    {
        ...
        public object ConvertToSerializable(object native)
        {
            ...
            var keyAdaptedType = TypeRegistry.GetAdapter(keyType)?.TargetType ?? keyType; // 获取适配器的目标类型
            var valueType = type.GetGenericArguments()[1];
            var valueAdaptedType = TypeRegistry.GetAdapter(valueType)?.TargetType ?? valueType; // 获取适配器的目标类型
            ...
            var adapterKey = TypeRegistry.HasAdapter(keyType)
                ? TypeRegistry.GetAdapter(keyType)
                : null;
            var adapterValue = TypeRegistry.HasAdapter(valueType)
                ? TypeRegistry.GetAdapter(valueType)
                : null;
        ...
        public object ConvertFromSerializable(object serializable)
        {
            ...
            var keyType = type.GetGenericArguments()[0];
            var keyAdaptedType = TypeRegistry.GetAdapter(keyType)?.TargetType ?? keyType;
            var valueType = type.GetGenericArguments()[1];
            var valueAdaptedType = TypeRegistry.GetAdapter(valueType)?.TargetType ?? valueType;
